...
const expected = {
  "name": "amazon",
  "version": "0.1.10",
  "url": "https://github.com/PedroAVJ/amazon",
  "dependencies": []
};
...
test("standalone plugin metadata is synchronized", async () => {
  ...
  await access(join(root, "preferences", "README.md"));
...
test("purchase preferences are private optional configuration", async () => {
  const skill = await readFile(join(root, "skills", "prepare-purchase", "SKILL.md"), "utf8");
  const preferences = await readFile(join(root, "preferences", "README.md"), "utf8");
  assert.match(skill, /AMAZON_PREFERENCES_DIR/);
  assert.match(skill, /never as purchase authority/);
  assert.match(preferences, /do not authorize/);
  assert.match(preferences, /present alternatives instead of silently substituting/);
  await assert.rejects(access(join(root, "preferences", "personal-care.md")));
});
